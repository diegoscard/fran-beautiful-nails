    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-lg border border-slate-200">
            ...
            {filterMode === 'month' ? (
                <input 
                    ...
                    className="px-2 py-1 bg-transparent border-none text-sm text-slate-700 font-medium focus:ring-0 cursor-pointer"
                />
            ) : (
                <input 
                    type="date" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 relative overflow-hidden">
          ...
          <h3 className="text-2xl font-bold text-emerald-600">{formatCurrency(totals.income)}</h3>
          <p className="text-xs text-emerald-600/80 mt-2 font-medium flex items-center gap-1">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-red-100 relative overflow-hidden">
          ...
          <h3 className="text-2xl font-bold text-red-600">{formatCurrency(totals.expense)}</h3>
          <p className="text-xs text-red-600/80 mt-2 font-medium flex items-center gap-1">
        ...
        <div className={`p-6 rounded-xl shadow-sm border relative overflow-hidden transition-colors ${
            ...
            : 'bg-gradient-to-br from-red-500 to-red-600 border-red-200 text-white'
        }`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all ${
                    type === 'income' 
                    ? 'bg-emerald-50 text-emerald-700 ring-2 ring-emerald-500 ring-offset-1' 
                    : 'bg-slate-50 text-slate-500 hover:bg-slate-100'
                  }`}
                >
            ...
            {transactions.length > 0 ? (
                ...
                <tbody className="divide-y divide-slate-100">
                    ...
                    <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3 text-sm text-slate-600 w-32">
                        {formatDate(t.date)}
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-slate-800">
                            {t.description}
                            {t.isService && <span className="ml-2 text-[10px] px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded-full border border-indigo-100">Atendimento</span>}
                        </td>
